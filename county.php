...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
  ...
  echo "<h2>" . $_GET['name'] . " County Daily Cases </h2>";
?>
...
and display for each date.*/
  ...
  $dates = $covid_county_data[$requestedCounty];
  $lastCases=0;
  foreach ($dates as $date) {
    if($date['state'] == $requestedState){
      $totalCases = $date['cases'];
      $newCases = $date['cases'] - $lastCases;
      $totalDeaths = $date['deaths'];
      echo '<tr>';
      echo '<td class="mdl-data-table__cell--non-numeric">'. $date['date'] .'</td>';
      echo '<td>'. number_format($totalCases) .'</td>';
      echo '<td>'. number_format($newCases) .'</td>';
      echo '<td>'. number_format($totalDeaths) .'</td>';
      echo '<td>';
